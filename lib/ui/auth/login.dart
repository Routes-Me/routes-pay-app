...
import 'package:routes_pay/locator.dart';
                      ...
                      children: [
                        ...
                        Container(
                          child: Row(
                            ...
                            children: [
                              provider.response.status == Status.LOADING ? CircularProgressIndicator() :

                              /*provider.response.status == Status.ERROR ? Center(
                                child: Text('Error'),
                              ) :*/
                               Expanded(
                                  flex: 1,
                                  child: ElevatedButton(
                                    ...
                                    onPressed: () async {
                                      if (validateForm.currentState.validate()){
                                        Map<String, String> params = {'username': emailController.text, 'password': encryption.encrypt(passwordController.text)};
                                        provider.setState(ApiResponse.loading(""));
                                        await provider.signIn(params);
                                        switch(provider.response.status){
                                          ...
                                          case Status.ERROR :
                                            return ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(provider.response.message)));
                                          case Status.INITIAL :
                                            return "";
                                          default:
                                            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(provider.response.message)));
                                        }
                                      }
                                    },
